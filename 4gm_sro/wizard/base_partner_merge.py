...
class base_partner_merge(osv.osv_memory):
    ...
    def action_merge(self, cr, uid, ids, context=None):
        ...
        @return : empty dict
        """
        res_adress = self.pool.get('res.partner.address')
        ...
        if hasattr(partner_pool, '_sql_constraints'):
            ...
            if c_names:
                ...
                for i in cr.fetchall():
                    ...
                    print "UNIQUE DATA",unique_data,"*****",unique_fields
                    str_unq = '---------------------------------------\n'
        ...
        res_id = partner_pool.read(cr, uid,part1)
        
        part_id = partner_pool.create(cr, uid, res, context=context)

        res_adress.write(cr, uid, res_id['address'],{'partner_id':part_id})
        keys = res_adress._columns.keys()

        dict_new = {}
        cont =  res['container']
        for k in cont:
            if k in keys:
                dict_new.update({k:cont[k]})
        res_adress.write(cr, uid, res_id['address'],dict_new)
                ...
                if hasattr(pool.get(model_raw), '_columns'):
                    ...
                    if pool.get(model_raw)._columns.get(name, False) and isinstance(pool.get(model_raw)._columns[name], fields.many2one):
                        ...
                        if name not in ('relation_partner_answer'):
                            print "update "+model+" set "+name+"="+str(part_id)+" where "+ tools.ustr(name) +" in ("+ tools.ustr(part1) +", "+tools.ustr(part2)+")"         
